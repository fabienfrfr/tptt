...
\subsection{Configuration d'Entraînement et Hyperparamètres}

L'entraînement est effectué sur 500 échantillons du jeu de données \texttt{yahma/alpaca-cleaned} sur 5 époques. Les séquences sont tokenisées à une longueur maximale de 384, avec troncature et padding si nécessaire. L'entraînement est effectué sur des GPU NVIDIA Tesla T4 (plateforme Kaggle), utilisant une précision mixte pour optimiser l'efficacité computationnelle. Les hyperparamètres clés incluent une taille de lot de 3, un taux d'apprentissage de $5 \times 10^{-4}$, et un clippage des gradients à 1,0 pour prévenir le débordement des gradients. La progression de l'entraînement est enregistrée toutes les 5 étapes, et les points de contrôle du modèle sont sauvegardés à la fin de chaque époque. Le nombre total d'étapes d'entraînement est de 420.
