...
In this work, $\beta_i$ is constructed via causal average pooling over the keys and values, which helps stabilize the scaling of the attention scores over long sequences. And we apply apply SiLU normalized in Q,K,V, RMSnorm for output and sigmoid for beta. % explicit calculus in this part (see the true implementation)
